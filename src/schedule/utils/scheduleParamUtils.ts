...
export default class ScheduleParamUtils {
    ...
    static getSeedDate(): ScheduleDate {
        ...
        return dateString !== null ? ScheduleDate.fromString(dateString) : (ScheduleTime.now().compareTo(new ScheduleTime(12 + 3, 15)) <= 0 ? ScheduleDate.now() : ScheduleDate.now().setDate(ScheduleDate.now().getDate() + 1));
    }
    ...
    static getViewMode(): ViewMode {
        ...
        switch (newRange) {
            ...
            case 'day':
                return ViewMode.Day;
            default:
                return ScheduleParamUtils.isMobile() ? ViewMode.Day : ViewMode.Week; // TODO: different default for thin screens (phones)
        }
    }
