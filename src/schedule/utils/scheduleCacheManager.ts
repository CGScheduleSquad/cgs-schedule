...
export default class ScheduleCacheManager {
    ...
    static getSchedule(calendarUrl: string): Promise<any> {
        if (localStorage === undefined) {
            // not supported
            console.log('Local storage is not supported! Loading schedule...');
            return this.reloadSchedulePromise(calendarUrl).then(jsonString => JSON.parse(jsonString));
        }
        ...
        if (scheduleObject.versionNumber === undefined || scheduleObject.versionNumber !== ScheduleAll.CURRENT_VERSION_NUMBER || scheduleObject.id !== calendarUrl || scheduleObject.schoolDivision !== scheduleParamUtils.getSchoolDivision()) {
        ...
        if (scheduleObject.creationTime === undefined || new Date().getTime() - scheduleObject.creationTime > 1000 * 60 * 60 * 24) {
            ...
            this.reloadSchedulePromise(calendarUrl).catch((reason: any) => {// save in the background
    ...
    private static reloadSchedulePromise(calendarUrl: string): Promise<string> {
        return ScheduleBuilder.generateScheduleFromBlockSources(
            calendarUrl,
            new VeracrossICSRawBlockSource(calendarUrl),
