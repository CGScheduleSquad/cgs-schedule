  ...
  allEvents.forEach(a => {
    ...
    if (index !== -1) {
      ...
      let block = {
        ...
        title: summary.join(" - "),
        subtitle: !description[0].hasOwnProperty("Block")
          ? ""
          : `Teacher • US ${description[0].Block.charAt(0) +
          (description[0].Block.charAt(1) === "L" ? " Long" : "") +
          (description[0].Block.charAt(1) === "X"
            ? " Flex"
            : "")} • ${room}`,
        rowSpan: 1
      };
  ...
  $.getJSON("blocks.json", function(data) {
    ...
    dayBlocks.forEach(day => {
      day.blocks.forEach(block => {
        if (["Lunch", "Break"].some(text => text === block.title)) {
          // TODO: hmm
          block.free = true;
        }

        if (
          block.startTime.getHours() === 11 &&
          block.startTime.getMinutes() === 25
        ) {
          // TODO: bruhh
...
function loadSchedule() {
  ...
  if (typeof Storage !== "undefined") {
    ...
    if (
      storageObject !== null &&
      currentCacheVersion === storageObject.version &&
      storageObject.uuid === calParameterValue
    ) {
      if (
        new Date().getTime() - storageObject.lastSaved <
        1000 * 60 * 60 * 24
      ) {
        // only timed out
        console.log(
          "Loaded, ms since saved: " +
          (new Date().getTime() - storageObject.lastSaved)
        );
        doTheStuffWithTheEvents(storageObject.allEvents, startDate, numDays);
      } else {
        console.log(
          "Loaded but outdated, ms since saved: " +
          (new Date().getTime() - storageObject.lastSaved)
        );
        doTheStuffWithTheEvents(storageObject.allEvents, startDate, numDays);
  ...
  navigationArrows
    ...
    .prop(
      "href",
      "?date=" +
      dateToVeracrossDate(previousDate) +
      "&range=" +
      newRange +
      "&cal=" +
      calParameterValue
...
function getEvents(thisMonday, saveIntoStorage, numDays, background = false) {
  ...
  const classUrl =
    "http://api.veracross.com/catlin/subscribe/" + calUUID + ".ics";
  Promise.all([corsGetPromise(classUrl)])
    .then(rawCals => {
      // temporary disable  // TODO: Do blocks need to be sorted? if so, sort them!!!
      // Promise.all([corsGetPromise(classUrl), corsGetPromise(calUrl)]).then(rawCals => {
      let allEvents = rawCals
        .map(rawCal => {
    ...
    .catch(any => {
      if (!background) {
        alert(
          "The request to load your calendar failed! Please check that you pasted the correct URL."
        );
        window.location.href = "./?";
      }
...
function getDatesForWeek(mondayDate, numDays) {
...
function httpRequestScheduleForDate(day) {
  // http request to veracross for the schedule blocks
  let letter =
    day.blocks.length !== 0 && day.blocks[0].letter !== undefined
      ? day.blocks[0].letter
      : "";
...
function appendDay(daySchedule) {
  ...
  $("table.sched.main > tbody > tr:nth-child(1)").append(`
    <td class="daylabel">
      <a href="https://portals.veracross.com/catlin/student/student/daily-schedule?date=${dateToVeracrossDate(
    daySchedule.date
  )}">
        <b>
          ${days[daySchedule.date.getDay()]} ${
    months[daySchedule.date.getMonth()]
  } ${daySchedule.date.getDate() +
  (!daySchedule.letter ? "" : ` (${daySchedule.letter})`)}
        </b>
      </a>
    </td>
    `);
  if (isEmptyDay(daySchedule)) {
    // append the day blocks
...
function filterBlocks(daySchedule) {
  // removes blocks not between 12 and 3 and duplicates
  daySchedule.blocks = daySchedule.blocks.filter(block => {
    ...
    if (isNaN(startHours) || startHours < 8 || startHours >= 12 + 3)
      return false;
    return !daySchedule.blocks.some(otherBlock => {
      // keep longer description one (TODO: show conflict)
      let sameTime =
        block.startTime.getTime() === otherBlock.startTime.getTime();
      return (
        sameTime &&
        (block.title.length < otherBlock.title.length ||
          block.subtitle.length < otherBlock.subtitle.length)
      );
    });
  });
  return daySchedule;
...
function appendRegularSchedule(daySchedule) {
  ...
  daySchedule.blocks.forEach(block => {
    ...
    normalTimes.some(time => {
      normalTimeIndex++;
      return (
        block.startTime.getHours() === time.getHours() &&
        block.startTime.getMinutes() === time.getMinutes()
      );
    });
    let smallBlock =
    ...
    let bgcolor =
      blockNumMatchAttempt !== null
        ...
        : block.free ||
        block.subtitle.match(/Break/) != null ||
        block.subtitle.match(/Lunch/) != null
        ? colorDict.free
        : colorDict[0];
    if (!block.free) {
    ...
    $(`table.sched.main > tbody > tr:nth-child(${normalTimeIndex})`).append(
      `<td rowspan="${block.rowSpan}" class="period mins${
        block.mins
      }" style="background: ${bgcolor};"><span class="coursename">${
        block.title
      }</span>${smallBlock ? "" : "<br>"}<span class="subtitle">${
        smallBlock ? "" : block.subtitle
      }</span><br></td>`
    );
  });
}
...
function prepRegularSchedule(daySchedule) {
  ...
  normalTimes.forEach(time => {
    // for each normal start time, push null to newBlocks if no blocks begin at the start time and push the block into newBlocks if it begins at the start time
    let blockWithTime = daySchedule.blocks.find(block =>
      compareTimes(block.startTime, time)
    );
    newBlocks.push(blockWithTime === undefined ? null : blockWithTime);
  });

  for (let i = 0; i < newBlocks.length; i++) {
    // Replace empty blocks with free blocks and extend long blocks into the next period
    if (newBlocks[i] === null) {
      let title = "Free";
      switch (i) {
        // case 3:
    ...
    if (
      ...
      (newBlocks[i].subtitle.match(/Long /) != null ||
        newBlocks[i].title === "Free" ||
        newBlocks[i].title === "Assembly") &&
      newBlocks[i + 1] == null
    ) {
      newBlocks[i].rowSpan++;
  ...
  newBlocks.forEach(block => {
    // calculate the duration of the block from the number of rows (time blocks) it spans
    block.mins = Math.min(
      Math.max(
        new Date(
...
function appendInlineSchedule(daySchedule) {
  $("table.sched.main > tbody > tr:nth-child(2)").append(
    ...
    if (block.endTime === undefined) {
      block.endTime =
        index === daySchedule.blocks.length - 1
          ? schoolEndTime
          : daySchedule.blocks[index + 1].startTime;
    }
  });
  ...
  daySchedule.blocks.forEach(block => {
    ...
    if (block.lateStart) {
      $(".special tbody")
        .last()
        .append(
          `<tr class="${minsClass}"><td colspan="2" class="period ${minsClass} specialperiod" style="background: ${colorDict.free};"><span class="coursename">${block.title}</span><br></td></tr>`
        );
      return;
    }

    let timeRange = `${format12HourTime(block.startTime)}-${format12HourTime(
      block.endTime
    )}`;
    ...
    $(".special tbody")
      .last()
      .append(
        `<tr class="${minsClass}"><td class="times ${minsClass}">${timeRange}</td><td rowspan="1" class="period ${minsClass} specialperiod" style="background: ${bgcolor};"><span class="coursename">${
          block.title
        }</span>${smallBlock ? "" : "<br>"}<span class="subtitle">${
          smallBlock ? "" : block.subtitle
        }</span><br></td></tr>`
...
function isNormalDay(daySchedule) {
  return daySchedule.blocks.every(block => {
    ...
    return normalTimes.some(
      time =>
        startHours === time.getHours() && startMinutes === time.getMinutes()
    );
  });
}
...
function parseVeracrossTime(timeString) {
  ...
  return new Date(
    ...
    (isPm && parseInt(splitString[0]) !== 12 ? 12 : 0),
