...
const normalTimes = [new Date(0, 0, 0, 8, 0), new Date(0, 0, 0, 8, 45), new Date(0, 0, 0, 9, 20), new Date(0, 0, 0, 9, 30), new Date(0, 0, 0, 9, 45), new Date(0, 0, 0, 10, 35), new Date(0, 0, 0, 11, 20), new Date(0, 0, 0, 11, 55), new Date(0, 0, 0, 12, 30), new Date(0, 0, 0, 13, 10), new Date(0, 0, 0, 13, 40), new Date(0, 0, 0, 14, 30)];
  ...
  $.getJSON('blocks.json', function(data) {
      ...
      if (unparsedDay !== undefined) {
        unparsedDay.forEach(block => {
          dayBlocks[index].blocks.push({
            letter: '', date: date, startTime: parseVeracrossTime(block.startTime), title: block.title, subtitle: '', rowSpan: 1
          });
        });
      }
    });
    ...
    dayBlocks.forEach(day => {
      day.blocks.forEach(block => {
        ...
        if (block.startTime.getHours() === 11 && block.startTime.getMinutes() === 25) {
          // TODO: bruhh
...
function loadSchedule() {
  ...
  if (typeof Storage !== 'undefined') {
    ...
    if (storageObject !== null && currentCacheVersion === storageObject.version && storageObject.uuid === calParameterValue) {
      if (new Date().getTime() - storageObject.lastSaved < 1000 * 60 * 60 * 24) {
        // only timed out
  ...
  navigationArrows
      .first()
      .prop('href', '?date=' + dateToVeracrossDate(previousDate) + '&range=' + newRange + '&cal=' + calParameterValue);
  navigationArrows
      .last()
      .prop('href', '?date=' + dateToVeracrossDate(nextDate) + '&range=' + newRange + '&cal=' + calParameterValue);
  ...
  $('td.controls.links a')
      .last()
      .prop('href', 'https://portals.veracross.com/catlin/student/student/daily-schedule?date=' + dateToVeracrossDate(seedDate));
}
      ...
      .then(rawCals => {
        ...
        if (saveIntoStorage) {
          ...
          localStorage.setItem('scheduleEvents', JSON.stringify({
            version: currentCacheVersion, uuid: calUUID, lastSaved: new Date().getTime(), allEvents
          }));
        }
        if (!background) {
          $('#loading').hide();
...
function prepRegularSchedule(daySchedule) {
  ...
  for (let i = 0; i < newBlocks.length; i++) {
    ...
    if (newBlocks[i] === null) {
      ...
      newBlocks[i] = {
        startTime: normalTimes[i], title: title, subtitle: '', rowSpan: 1, free: true
      };
    }
...
function appendInlineSchedule(daySchedule) {
  ...
  if (daySchedule.blocks[0].startTime.getTime() !== normalTimes[0].getTime()) { // add free block first
    daySchedule.blocks.unshift({
      startTime: normalTimes[0], endTime: daySchedule.blocks[0].startTime, title: 'Free', subtitle: '', rowSpan: 1, lateStart: true
    });
  }
  ...
  daySchedule.blocks.forEach((block, index) => { // add free blocks to fill in the gaps
    ...
    if (nextTime.getTime() - block.endTime.getTime() > 1000 * 60 * 10) {
      newBlocks.push({
        startTime: block.endTime, endTime: nextTime, title: 'Free', subtitle: '', rowSpan: 1, free: true
      });
    }
  });
  daySchedule.blocks = newBlocks;
  ...
  daySchedule.blocks.forEach(block => { // append the blocks
    ...
    if (block.lateStart) {
      $('.special tbody')
          .last()
          .append(`<tr class="${ minsClass }"><td colspan="2" class="period ${ minsClass } specialperiod" style="background: ${ colorDict.free };"><span class="coursename">${ block.title }</span><br></td></tr>`);
      return;
    }
    ...
    $('.special tbody')
        .last()
        .append(`<tr class="${ minsClass }"><td class="times ${ minsClass }">${ timeRange }</td><td rowspan="1" class="period ${ minsClass } specialperiod" style="background: ${ bgcolor };"><span class="coursename">${ block.title }</span>${ smallBlock ? '' : '<br>' }<span class="subtitle">${ smallBlock ? '' : block.subtitle }</span><br></td></tr>`);
  });
}
